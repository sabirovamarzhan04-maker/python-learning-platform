...
app.mount("/static", StaticFiles(directory="дипломка", html=True), name="static")
@app.get("/")
async def home():
...
def generate_tasks(req: TaskRequest):
    ...
    context_msg += (
        "Маңызды: Тек қана 'questions' кілті бар кірістірілген JSON объектісін қайтар.\n"
        "Мысалы: {\"questions\": [{\"type\": \"test\", \"question\": \"...\", \"options\": [\"A\",\"B\",\"C\",\"D\"], \"correct\": 1}]}\n"
        "Әр сұрақ объект болсын: "
        "{ \"type\": \"test\" | \"code\" | \"theory\", "
        "\"question\": \"...\", "
        "егер test болса: \"options\": [\"A\",\"B\",\"C\",\"D\"], \"correct\": 1-ден басталатын индекс; "
        "егер code болса: \"template\": \"код с ______\", \"answer\": [\"дополнения\"]; "
        "егер theory болса: қысқа ашық сұрақ. "
        "Барлық 10 сұрақ әртүрлі болсын."
    )
